...
function buildDB() {
  var newDb = new sqlite3.Database(getUserHome() + "/cedict/cedict.sqlite3");
  var fileContents = fs.readFileSync(getUserHome() + '/cedict/cedict_ts.u8');
  ...
  var regex = /(.*?) (.*?) \[(.*?)\] \/(.*)\//;
  newDb.run("CREATE VIRTUAL TABLE dict USING fts4(traditional VARCHAR(50), simplified  VARCHAR(50), pinyin VARCHAR(100), english VARCHAR(500) )"); 
  newDb.serialize(function() {
    ...
    transact.finalize(function() {
      process.stdout.write("\nDone!");
      newDb.close();
      fs.unlink(getUserHome() + '/cedict/cedict_ts.u8');
      repl();
    });
  });
}
...
function findDefinitions(line) {
  ...
  var selector = "*" + line.replace(/ /g, '*') + "*";
  console.log(selector);
  db.each("SELECT * FROM dict WHERE dict MATCH ? LIMIT 5;", selector, function(err, entry) {
    if(err) {
      console.log(err);
      return;
    }
